...
class Lane():
    def __init__(self,previous_margin,nwindows,windowmargin,minipix):
        self.left_line = self.Line()
        self.right_line = self.Line()
    ...
    def find_lane_pixels_search_previous(self,binary_warped):
        ...
        margin = self.margin
        #grab the polyfit from last frame 
    ...
    def fit_polynomial(self,binary_warped):
        ...
        if(self.detected == False):        
            ...
            #print("find_lane_pixels_sliding_window")
 
        else:
            #print("find_lane_pixels_search_previous")
    ...
    def measure_curvature_pixels(self,left_fit,right_fit,y_eval):
    ...
    def lane_fit_sanity_check(self,left_curve,right_curve):
        ...
        if((self.left_line.radius_of_curvature!=None) & (self.right_line.radius_of_curvature!=None)):
            #check line curve is almost the same (30 degree difference is ok)
            if(abs(left_curve-self.left_line.radius_of_curvature)>0.5):
                status=False
            if(abs(right_curve-self.right_line.radius_of_curvature)>0.5):
                status=False

        
        return status
    
    def get_lane_highlighted(self,binary_warped):
      

        left_fit,right_fit =self.fit_polynomial(binary_warped)
        
        left_curve,right_curve = self.measure_curvature_pixels(left_fit,right_fit,binary_warped.shape[0])
        
        
        #check if new fit makes sense 
        if(self.lane_fit_sanity_check(left_curve,right_curve)):
            #save current line fit
            self.left_line.current_fit   =left_fit
            self.right_line.current_fit  =right_fit
            #save current curvature
            self.left_line.radius_of_curvature = left_curve
            self.right_line.radius_of_curvature = right_curve
        else:
            #reset to use the sliding window method and use old values 
            left_fit= self.left_line.current_fit   
            right_fit= self.right_line.current_fit
            left_curve=self.left_line.radius_of_curvature 
            right_curve = self.right_line.radius_of_curvature
            self.detected = False
